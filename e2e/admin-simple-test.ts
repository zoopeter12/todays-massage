/**
 * Simple Admin E2E Test Script
 * Run with: npx ts-node e2e/admin-simple-test.ts
 */
...
interface TestResult {
  ...
  httpStatus?: number;
  loadTime?: number;
  buttons?: { text: string; visible: boolean; enabled: boolean }[];
  issues: string[];
}
...
async function runTests() {
  ...
  for (const pageInfo of ADMIN_PAGES) {
    ...
    try {
      ...
      const buttonResults: { text: string; visible: boolean; enabled: boolean }[] = [];
      ...
      if (pageContent.includes('Error') || pageContent.includes('error')) {
        ...
        for (const el of errorElements) {
          ...
          if (text?.trim()) {
            issues.push(`Error element found: ${text.trim().substring(0, 100)}`);
          }
        }
      }
